...
테스트 파일 49개(`conftest.py` 별도), 테스트 1,023개다(2026-09-23 실측, Python 3.12.14).
`local/.venv/` 와 `~/.warruru/` 가 이미 있다.

**이 머신에서는 git 을 부르는 17개가 실패한다** — `test_gitinfo` · `test_publish_git_repo`
· `AC07`. 코드가 아니라 `/usr/bin/git` 이 Xcode 라이선스 미동의로 멈춰 있어서다.
`sudo xcodebuild -license accept` 한 번이면 사라진다. **이 실패를 코드 탓으로 진단하지 마라.**
...
- 화면 — `/d/{date}` · `/c/{YYYY-MM}` · `/t` · `/t/{slug}` · `/drafts/{id}`
  ...
  **대화마다 세션이 하나다**(v7, 2026-09-16) — 왼쪽 목록에서 지난 대화를 골라
  ...
  에이전트 옆에서 **모델도 고른다**(`asking.MODELS`, 목록 밖은 400).
  에이전트를 부르는 화면에는 **[중지]** 가 있고, 끊기면 자식을 프로세스 그룹째
  죽인다. [오늘 정리] 가 하루치를 세 절로 접는다.
  그 정리를 [기록으로 올리기] 로 `learning_record` 에 넣는다 — **① 절만**,
  주제는 골라서(명세 §2.6·§2.7·§2.8)
  조회는 토큰 불필요, 상태 변경 폼만 토큰.
  **허브는 균등 카드 그리드다**(2026-09-08 확정) — 같은 크기 카드 여섯 개가
  각각 상위 셋만 보여주고 나머지는 눌러 들어간다. 목록 전체를 허브에 펼치면
  긴 칸과 짧은 칸의 높이 차로 화면 절반이 빈다. 마감은 자격증·공고를 섞어 오고,
  ...
  **예외 하나: 초록은 접수 여부에만 쓴다**(2026-09-22, 사용자가 정함).
  접수는 "했다 / 안 했다" 가 한눈에 갈려야 하는데 빨강은 이미 마감을 맡고
  있어서 그 둘을 같은 색으로 칠할 수 없다. **다른 화면으로 번지면 셋 다
  신호를 잃는다** — `test_색이_둘을_넘지_않는다` 가 그 경계를 지킨다.
...
**그 뒤 2026-09-06 ~ 09-22 에 실제로 쓰면서 붙인 것들**(명세 §2.6~§2.15 에 규칙,
PR #72~#113). 화면이 "오늘 뭐 하지" 에 답하게 만드는 일이 축이었다 —
묻고 답하기(구독 CLI 자식 프로세스) · 공부 대화와 기록 승격 · 책(목차 · 장 요약 ·
오늘 읽기) · CS 답해보기 · 자격증(오늘 할 일 · 접수일) · 기술스택 축 ·
홈(오늘 할 것 · 스트릭 · 달력) · 기록에서 글 만들기와 다듬기 ·
대화 세션(v7) · 모델 선택 · [중지] · 대화 목록 20개 + 더 보기 ·
회차별 접수 체크(v8) · 시험장 도구 연습 체크 · **UML·ERD 연습판**
(라이브러리 없는 SVG 하나 — 명세 §2.11 i).
