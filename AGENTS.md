...
- 화면 — `/d/{date}` · `/c/{YYYY-MM}` · `/t` · `/t/{slug}` · `/drafts/{id}`
  ...
  **화면은 계기판이다**(2026-09-07 재설계) — 마감이 자격증·공고를 섞어 먼저 오고,
  준비도는 막대가 아니라 **빈칸 격자**다. 막대는 "얼마나 왔나", 격자는
  "몇 칸이 비었나" 를 말하는데 이 도구가 답하려는 것은 후자다.
  **웹폰트를 부르지 않는다**(로컬 우선). 표면은 테두리가 아니라 여백과
  흰 카드로 나누고, 모노는 **숫자에만** 남긴다. 상태에 쓰는 색은 둘뿐이다 —
  파랑(누를 수 있다·채워졌다) · 빨강(마감·0). 셋이 되면 어느 것도 신호가
  못 되고, 실제로 그렇게 한 번 실패했다.
  JavaScript 는 초안 복사와 일반/다크 모드 전환, 묻기 SSE 에만 쓴다.
