...
**지금 이 저장소의 상태 (2026-09-22)**

이 프로젝트는 "코드보다 명세를 먼저"라는 원칙으로 시작했고, 그 방식은 절반만 통했다.
`local/` 하나는 명세대로 끝까지 구현돼 실제로 돌아간다 — 소스 12,266줄(템플릿 포함 17,924),
테스트 파일 49개, **테스트 1,020개.**
기록에서 초안 파일까지 한 바퀴는 실제 데몬을 띄운 채로 돌려서 확인했다.
2026-09 에는 매일 쓰면서 화면을 "오늘 뭐 하지" 에 답하도록 고쳐 나갔다 —
홈의 오늘 할 것과 스트릭, 책·자격증·기술스택 축, 그리고 화면 안에서 구독 CLI
(Codex · Claude)에게 묻는 공부 챗봇이다. 챗봇은 대화마다 세션이 따로 남아
지난 대화를 골라 이어 물을 수 있고, 모델을 고르고 도중에 [중지] 할 수 있다.
...
을 열면 홈(오늘 할 것)으로 간다. 화면은 네 갈래다 —
**오늘**(`/`), **기록**(`/t` · `/t/{slug}` · `/d/{date}` · `/c/{YYYY-MM}` · `/drafts/{id}`),
**책**(`/career/books` · `/career/book/{책}` · `/career/book/{책}/today`),
**취업 준비**(`/career` 허브 · `/career/certs` · `/career/stacks` · `/career/companies`). 에이전트의 MCP 설정에 `warruru-mcp` 를 등록해 두면
