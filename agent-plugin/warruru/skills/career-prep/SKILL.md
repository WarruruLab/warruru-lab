...
- **`essays` 는 `문항 | 글자수 | 붙일 슬러그, 슬러그` 다.** 문항마다 붙일
  기록이 몇 건인지 화면이 센다. 슬러그를 안 적어도 문항은 보인다.
  **문항은 본 것만 적는다** — 작년 문항을 올해 것처럼 쓰지 않는다.

**한 회사에 공채가 여럿이어도 노트는 한 장이다**(2026-09-01 확정).
3월 공채와 9월 공채의 요구 기술을 `required` 에 합집합으로 넣는다. 쪼개면
"이 회사에 얼마나 준비됐나" 에 답이 안 나온다 — 공채별 이력은 노션에 남는다.
...
**축마다 화면이 답하는 질문이 다르다**(2026-09-01 확정).

| 축 | 화면이 먼저 답하는 것 | 한 장의 단위 |
|---|---|---|
| 자격증 | 언제 접수하나 | 자격증 하나(단계별) |
| CS 지식 | 답할 수 있나 | 묶음 하나 |
| 로드맵 주제 | 어디까지 왔나 | 묶음 하나(**로드맵 순서**) |
| 공고 | 무엇을 요구하나 · 자소서에 뭘 쓰나 | 회사 하나 |

**로드맵 묶음은 진도가 맨 위에 오고 목록이 로드맵 순서로 선다.** 주차 번호는
세지 않는다 — 밀리면 "늦었다" 가 매일 보이고, 그건 로드맵 문서가 하지 않기로
한 일이다. 순서만 안다.

**CS 지식은 "면접 문서" 다.** 화면을 여는
...
`ask topic={슬러그}` 한 줄을 받으면:

1. 그 주제의 노트(`topics/{슬러그}.md`)와 참고 링크를 먼저 읽는다.
2. 답한다. **원문을 옮기지 말고 물어본 것에만 답한다.**
3. `~/.warruru/career/answers/{슬러그}/{YYYY-MM-DD}.md` 로 저장한다.
...
- **데몬은 모델을 부르지 않는다.** 묻는 쪽은 사용자 앞의 에이전트이고 데몬은
  파일을 읽어 보여줄 뿐이다. 그래서 네트워크가 끊겨도 주제 화면은 열린다.
- **받은 답은 기록이 아니다.** 기록은 *내 말로 정리했다* 이고 이건 *물어서
  받았다* 이다. `record_learning` 을 대신 불러 주지 마라 — 사용자가 자기
  말로 다시 쓸 때가 기록할 때다.
- 같은 날 여러 번 물으면 같은 파일에 이어 쓴다.

## 회사 인재상

회사 노트 앞머리 `values` 에 쉼표로 적는다. **공고와 설명회에서 확인한
것만.** 기술만 맞추고 이쪽을 모르면 자소서와 면접에서 그대로 드러난다.

```
values: 직무 중심 채용, 책임 의식, 협업, AI 활용 능력
```

## 자격증 커리큘럼

`curriculum` 은 `단계 | 항목 | 전체 수량` 이다. 화면이 남은 분량을 다음
일정까지 남은 날로 나눠 **오늘 분량**을 계산한다.
...
**단위를 섞지 마라.** 회차와 문항을 한 항목에 넣으면 나눈 숫자가 뜻을 잃는다.
진도는 사용자가 화면에서 누른다 — 에이전트가 대신 올리지 않는다.
