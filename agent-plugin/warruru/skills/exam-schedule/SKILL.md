...
exams:
  ...
  - 2026-09-21..2026-09-23 | 3회 실기 접수 | 비고 | | 실기   # 기간은 시작..끝
---
```

## 틀리면 조용히 아픈 것 넷

0. **접수처럼 기간이 있는 일정은 `시작..끝` 으로 적어라.** 시작일 하나만 적으면
   접수가 **시작된 날** 그 줄이 지난 것이 되어 화면에서 사라진다 — 정확히
   접수할 수 있는 동안에만 안 보인다. 2026-09-22 에 정보처리기사 실기 접수를
   하루 차이로 놓칠 뻔했고, 그래서 기간 표기가 생겼다. 기간이 열려 있는 동안
   화면은 "내일 마감" 처럼 **마감까지**를 센다.
...
2.5. **접수했는지는 노트에 적지 않는다.** 화면의 [접수함] 버튼이 DB 에 남긴다.
   에이전트가 노트를 고쳐 표시하려 하면 두 곳이 어긋난다.
