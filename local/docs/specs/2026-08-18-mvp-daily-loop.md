...
- **입력:** 선택 `date=YYYY-MM-DD`(조회, 생략하면 오늘). `/t` 는 선택 날짜
  자정~자정 구간 + **그 밖의 주제 전체**,
  `/t/{slug}` 는 그 슬러그의 전체 기록.
- **출력(`/t`):** 상단 날짜 양옆에 전날·다음날 화살표와
  `오늘 기록 N건` 또는 `선택 날짜 기록 N건`, slug 별 건수 · kind 배지 ·
  마지막 기록 시각.
...
### 2.6 주제 화면에서 묻는다 (2026-09-06 개정으로 추가)

**요구:** `/t/{slug}` 에서 그 주제에 대해 묻고, 답이 같은 화면에 흐르고,
이어서 물으면 **앞의 대화를 기억한다.** 터미널을 열지 않는다.

- 입력은 화면의 한 칸이다. 답은 도착하는 대로 흐른다(SSE).
- **주제마다 스레드 하나.** `db-index` 에서 이어 물으면 `db-index` 의 맥락이고,
  `algo-dp` 는 별개다. 주제가 이 도구의 축이므로 대화의 축도 그것이다.
- 답이 끝나면 `~/.warruru/career/answers/{slug}/{날짜}.md` 에 붙는다.
  **새 자리를 만들지 않는다** — 에이전트가 터미널에서 답했을 때와 같은 자리다.
- 기록(`record_learning`)은 자동으로 만들지 않는다. 물어서 받은 것은 참고 자료이고,
  기록은 사용자가 자기 말로 다시 썼을 때다(§2.1 과 같은 구분).

**데몬은 여전히 모델을 모른다.** 부르는 것은 이미 로그인된 CLI 이고,
데몬이 하는 일은 그것을 자식 프로세스로 띄워 표준출력을 읽는 것뿐이다.
모델 이름 · 프롬프트 조립 · 재시도 · 인증 · 토큰 갱신은 전부 CLI 안에 있고
이 저장소에 들어오지 않는다.

**깨지 않는 경계 셋**

- **초안 조립기는 LLM 호출 0 을 유지한다.** 6단 조립은 결정적이다(§2.4).
  이 절이 다루는 것은 '묻고 답하기' 뿐이고, 초안 경로에 손대지 않는다.
- 데몬은 SQLite 의 유일한 writer 로 남는다.
- `publish/` 는 여전히 `sqlite3` 와 `store.*` 를 모른다.

**알고 받는 비용 넷**

- 화면이 **구독 한도를 쓴다.** 브라우저에서 누르는 것이 곧 토큰이다.
  버튼 하나가 한 번의 왕복이므로, 실수로 눌러도 한 번이다.
- CLI 가 없거나 로그인이 만료되면 실패한다. **화면이 그 사실을 그대로 말한다** —
  "잠시 후 다시" 로 덮지 않는다. 고치는 방법(`codex login`)이 화면에 있다.
- 이 경로가 죽어도 **기록 · 초안 · 발행은 안 흔들린다.** 자식 프로세스가
  실패하면 이 화면만 실패한다.
- 답이 오는 데 수십 초가 걸린다. 그래서 SSE 이고, 그래서 한 번에 하나만 돈다.

### 2.7 공부하며 정리한다 (2026-09-08 개정으로 추가)

§2.6 을 셋으로 넓힌다. 근거는 쓰임새다 — 공부는 주제 하나가 아니라
**"이 책을 읽는 중"** 으로 흐를 때가 많은데, 그 자리에 물을 칸이 없으면
터미널로 나가야 하고 나가면 안 돌아온다.

**(a) 어디서든 묻는다.** 주제(`/t/{slug}`) · 책(`/career/book/{key}`) ·
묶음(`/career/stack/{key}`) 세 화면이 **부분 템플릿 하나**를 쓴다.
각자 그리면 어느 하나에만 정리 버튼이 빠지는 식으로 갈라진다.
대화 키는 `{축}-{열쇠}` 다(`book-kafka-practice`) — 주제 슬러그와 안 겹친다.

**(b) 하루치를 접는다.** [오늘 정리] 는 같은 스레드에 정해진 프롬프트를
보내 세 절로 접는다 — ① 오늘 알게 된 것 ② 아직 막히는 것 ③ 다음에 볼 것.

**프롬프트는 코드에 둔다**(`routes_web.SUMMARY_PROMPT`). 화면에서 만들면
버전도 테스트도 안 붙고 두 화면이 조금씩 다른 문장을 보내기 시작한다.
그 문장의 핵심은 한 줄이다 — **"내가 이해했다고 말한 것만 ① 에 넣어라."**
에이전트가 설명한 것을 내가 안 것으로 적으면 그 파일은 공부 기록이 아니라
대화 사본이고, 면접장에서 그대로 드러난다.

**(c) Codex 와 Claude 를 고른다.** 화면의 드롭다운으로 고른다.
**이벤트 모양이 서로 다르다**(2026-09-08 실측) — Codex 는
`thread.started` / `item.completed` / `turn.completed`, Claude 는
`system.init` / `assistant` / `result` 다. `asking.translate` 한 곳에서
같은 이름으로 옮기므로 화면은 어느 CLI 인지 모른다.

**CLI 를 바꾸면 대화를 잇지 않는다.** 스레드 id 는 그 CLI 안에서만 뜻이
있어서 넘겨주면 조용히 새 대화가 열리거나 실패한다. `ask_thread.cli` 가
저장된 값과 다르면 새로 시작한다.

**알고 받는 것** — 답 파일에 정리와 문답이 같은 파일에 섞인다.
`## 오늘 정리` 로 구분만 해 둔다. 나누는 것은 읽어 보고 불편할 때 한다.

### 2.8 정리를 기록으로 올린다 (2026-09-08 개정으로 추가)

받은 답은 **참고 자료**이고 기록은 *내 말로 정리했다* 인데(§2.1),
그 사이를 건너는 다리가 없었다. 답이 쌓이는 동안 기록은 따로 남겨야 했고,
그래서 답 203건에 면접 문장은 43건뿐이다.

**챗봇이 그 다리를 놓는다.** [오늘 정리] 뒤에 [기록으로 올리기] 가 뜬다.

- **① 절만 올라간다.** ②(막히는 것)·③(다음에 볼 것)은 아직 답 못 하는
  것이라 기록이 아니다. 기록으로 올리면 준비도 막대가 거짓말을 한다.
- **주제를 고르게 한다.** 책 하나가 주제 열한 개를 덮는데 임의로 고르면
  틀린 자리에 쌓이고, 틀린 자리에 쌓인 기록은 아무 화면에서도 안 보인다.
  고를 목록은 그 화면이 덮는 슬러그다.
- **본문은 고칠 수 있다.** 받아 적은 문장은 면접에서 안 나온다.
- `interview` 를 같은 폼에서 받는다. 지금 15% 인 그 필드다 —
  여기서 안 채우면 계속 안 찬다.
- 쓰기는 **`learning.record` 를 그대로 쓴다.** MCP 가 부르는 것과 같은
  함수라 슬러그 정규화도 힌트도 멱등도 한 곳에만 있다. `tool` 은 `web` 이다.
...
- **`GET /t?date={YYYY-MM-DD}`** — 날짜별 주제 목록. 날짜는 생략하면 오늘이며,
  맨 위에서 전날·다음날로 이동한다. `오늘 기록 N건`은 0건이면 붉게.
...
```
codex exec --json -C <홈> -s read-only --skip-git-repo-check <프롬프트>
codex exec resume <thread_id> --json -C <홈> -s read-only <프롬프트>
```

1. **`stdin` 을 닫는다.** 열어 두면 `Reading additional input from stdin...`
   에서 **영원히 멈춘다**(2026-09-05 실측, 4분 타임아웃).
2. **`stdout` 만 파싱한다.** `stderr` 에 CLI 로그가 섞여 나오고 JSONL 이 아니다.
   그쪽은 `~/.warruru/logs/` 로 보낸다.
...
이어 묻기는 이 값으로 `resume` 한다. 세션 본문은 CLI 가
`~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl` 에 들고 있고
**이 저장소는 그 파일을 읽지 않는다.** 우리가 드는 것은 id 하나뿐이다.
...
- ~~데몬 안의 LLM 호출(Ollama / OpenAI)~~ — **2026-09-06 개정으로 부분 해제.**
  원래 이유는 "서비스 기동·모델·프롬프트·타임아웃·재시도가 전부 새로 필요하고
  1주 이상" 이었는데, **그 전부가 이미 로그인된 CLI 안에 있다.**
  데몬이 모델을 부르는 것이 아니라 `codex exec` 를 자식 프로세스로 띄운다 —
  새 런타임 의존성 0 · 새 포트 0 · 새 상주 프로세스 0 이고, 인증은
  `~/.codex/auth.json` 의 OAuth 토큰이라 이 저장소에 비밀이 들어오지 않는다.
  범위는 §2.6 의 '묻고 답하기' 하나로 **좁게 연다.**
  **초안 조립기의 LLM 호출 0 은 그대로다** — 그것이 원래 지키려던 것이었다.
  Ollama 같은 별도 서비스 기동은 여전히 범위 밖이다.
