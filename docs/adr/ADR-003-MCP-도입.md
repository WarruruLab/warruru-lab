...
현재 '개발톡(DevTalk)'의 핵심 기능인 문제 해결 채팅 기능 개발이 완료 단계에 접어들었다. 다음 단계로 해결된 세션 로그를 바탕으로 기술 블로그 글을 자동 생성해 주는 '문제와르르(DevLog)' 서비스를 개발해야 하는 시점이다.
...
3. **DevLog (`warruru-service`)**: 블로그 글 생성 및 가공을 담당하는 서비스.
...
- **DB 공유**: 'DevTalk', 'DevLog', 'MCP' 모두 동일한 DB 테이블(`Session`)을 바라봐야 한다. `Core` 모듈에 Entity를 정의함으로써 코드 중복을 제거하고 데이터 무결성을 유지할 수 있다.
