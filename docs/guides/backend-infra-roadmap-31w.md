...
- 슬러그를 고치거나 더하면 `topics.py` 상수도 같이 고친다. 자동 반영은 없다.
  (테스트 `test_로드맵_문서의_슬러그가_전부_들어_있다` 가 양방향으로 대조하므로,
  한쪽만 고치면 바로 빨간불이 켜진다.)
- **기록할 때는 여기 적힌 슬러그를 그대로 `topic` 에 넣는 편이 안전하다.**
  정규화는 기호를 살리지 않는다 — `JPA N+1` 이라고 적으면 `jpa-n1` 이 되어
  권장 슬러그 `jpa-n-plus-one` 과 영영 다른 주제가 된다.
  한글로 적으면 한글 슬러그(`커넥션 풀` → `커넥션-풀`)가 되므로,
  이 목록의 영문 슬러그와는 절대 만나지 않는다. 한쪽으로 정해 쓴다.
